...
*  This program uses OpenMP to parallelize x × A = b matrix-vector multiplication
*/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
int main(int argc, char *argv[]) {
    ...
    // Parallelize the initialization of the vector x with values in range [0, 1]
    for (int i = 0; i < n; i++) {
        x[i] = (double)rand() / RAND_MAX;
    }

    // Parallelize the initialization of the matrix A with values in range [0, 1]
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
    ...
    // Parallelize the matrix-vector multiplication to compute b = x * A
    ...
    double end = omp_get_wtime();  // Stop measuring the time
    ...
    printf("]\nExecution time: %.4f seconds\n", end - start);  // Print the execution time
